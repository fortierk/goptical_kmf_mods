...
namespace _Goptical {
  ...
  namespace Sys {
    ...
    SourceObjNA::SourceObjNA(const Math::Vector3 &object)
      : Source(object),
        _rays(_rays_storage)
    {
    }

    void SourceObjNA::wavelen_ref_inc(double wl)
    {
    ...
    void SourceObjNA::wavelen_ref_drop(double wl)
    {
    ...
    void SourceObjNA::add_chief_rays(const Sys::System &sys)
    {
      add_chief_rays(sys.get_entrance_pupil());
    }

    void SourceObjNA::add_chief_rays(const Sys::Surface &s)
    {
    ...
    void SourceObjNA::add_marginal_rays(const Sys::System &sys, double entrance_height)
    {
      add_marginal_rays(sys.get_entrance_pupil(), entrance_height);
    }

    void SourceObjNA::add_marginal_rays(const Sys::Surface &s, double entrance_height)
    {
    ...
    void SourceObjNA::add_rays(const Math::VectorPair3 &ray, const Element *ref)
    {
      GOPTICAL_FOREACH(l, _spectrum)
        {
    ...
    void SourceObjNA::add_ray(const Light::Ray &ray, const Element *ref)
    {
    ...
    void SourceObjNA::clear_rays()
    {
      _rays.clear();
      _wl_map.clear();
    }

    void SourceObjNA::generate_rays_simple(Trace::Result &result,
    ...
    void SourceObjNA::generate_rays_intensity(Trace::Result &result,
                                             const targets_t &entry) const
    {
